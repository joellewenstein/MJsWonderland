...
module Warden
  ...
  class Proxy
    ...
    def set_user(user, opts = {}) 
      ...
      # HACK: We need to skip this line, so that it returns to Line231, so that it calls _after_authenticate
      # Note it's possible this will fail on confirmable.rb, which needs it. If so, try and branch on what strategy is active
      
      # Run the after hooks for setting the user  
      #Warden::Manager._after_set_user.each{|hook| puts hook; hook.call(user, self, opts)}
    ...
    def _perform_authentication(*args)
      ...
      strategies.each do |s|
        ...
        puts "Strategy #{s} - valid? #{strategy.valid?}"
      ...
      puts "Winning strategy: #{winning_strategy}"
      puts "winning_strategy.user : #{winning_strategy.user}"
